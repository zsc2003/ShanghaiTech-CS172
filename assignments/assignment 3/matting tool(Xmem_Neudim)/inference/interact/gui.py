...
class App(QWidget):
    def __init__(self, net: XMem, 
        ...
        #szh add
        self.import_picture= QPushButton('Import Picture')
        self.import_picture.clicked.connect(self.on_import_picture)
        #szh end
        # LCD
        self.lcd = QTextEdit()
        ...
        navi.addWidget(self.import_picture)
    ...
    #szh add
    def on_import_picture(self):
        #用一个弹窗来获得想要得到的文件夹路径
        #file_path=QFileDialog.getExistingDirectory(self,'选择文件夹','./')
        file_path=QFileDialog.getExistingDirectory(self,'选择文件夹','D:/NeuDim/NeuDim_data/test_seg') # default open path
        print(file_path)
        if file_path == '':
            return
        self.config["images"] = file_path
        self.res_man=ResourceManager(self.config)
        self.height, self.width = self.res_man.h, self.res_man.w
        # current frame info
        self.curr_frame_dirty = False
        self.current_image = np.zeros((self.height, self.width, 3), dtype=np.uint8) 
        ...
        self.num_frames = len(self.res_man)
        self._try_load_layer('./docs/ECCV-logo.png')
        self.console_push_text('Import success.')
        self.tl_slider.setMinimum(0)
        self.tl_slider.setMaximum(self.num_frames-1)
        self.tl_slider.setValue(0)
        self.tl_slider.setTickPosition(QSlider.TicksBelow)
