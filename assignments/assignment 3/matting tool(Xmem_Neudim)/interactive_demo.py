...
if __name__ == '__main__':
    ...
    parser.add_argument("--dont_export_in_situ", action = "store_false", help = "type to stop export masks and matted images in original folder")
    parser.add_argument("--no_matting", action = "store_false", help = "stop auto matting")
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        # Manages most IO
        if(config["workspace"] is None and config["images"] is None and config["video"] is None):
            app = QApplication(sys.argv)
            ex = App(network, None, s2m_controller, fbrs_controller, config)
            sys.exit(app.exec_())
        else:
